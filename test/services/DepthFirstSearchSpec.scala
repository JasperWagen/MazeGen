...
class DepthFirstSearchSpec extends AnyFunSuite{
    ...
    test("00 search, check null case"){
        val inputMaze: Array[Array[Byte]] = Array(Array())
    ...
    test("05 search, check 10x9, forked path"){
        val inputMaze: Array[Array[Byte]] =
            Array(
    ...
    test("06 convert searchPath to Array, 1x1 case"){
        ...
        val searchArr:Array[Array[Byte]]= Array(
            Array(1))
        ...
        for (i <- inputMaze.indices) {
            for (j <- inputMaze(0).indices) {
                assert(depthFirstSearch.mapSearchPathToArray(searchPath, mazeDimensions)(i)(j) == searchArr(i)(j))
            }
        }
    }
    ...
    test("06 convert searchPath to Array, 2x2 case"){
        ...
        val searchArr:Array[Array[Byte]] =
            Array(Array(1, 1),
                  Array(0, 0))
